...
if (isset($_FILES['profileImage']) && $_FILES['profileImage']['error'] == 0) {
    ...
    if (in_array($ext, $allowed) && $file['size'] <= 2*1024*1024) {
        $newFileName = 'admin_' . $user_id . '.' . $ext;
        $uploadDir = 'uploads/';
...
if (isset($_POST['action']) && $_POST['action'] == 'updateProfile') {
    ...
    if (empty(trim($bio))) {
        $bio = 'لم يتم إضافة نبذة عن المسؤول بعد.';
    }
...
$bio = $user['bio'] ?? 'مسؤول عن تنظيم ومتابعة الفعاليات والأندية، وضمان سير المنصة بسلاسة.';
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>مِـداد | Medad - لوحة التحكم</title>
...
<main>
  <section class="profile-container">
    ...
    <div class="profile-image-container">
      <img id="profileImage" src="<?= $profileImage ?>" alt="صورة المسؤول">
    ...
    <h2 id="adminName"><?= htmlspecialchars($name) ?></h2>
    <p id="adminBio"><?= htmlspecialchars($bio) ?></p>
    ...
    <form id="editForm" class="edit-form" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <textarea id="editBio" name="bio" class="form-input" rows="4" placeholder="اكتب نبذة عنك هنا..."><?= htmlspecialchars($bio === 'لم يتم إضافة نبذة عن المسؤول بعد.' ? '' : $bio) ?></textarea>
      </div>
...
<script>
// JavaScript for admin profile functionality
