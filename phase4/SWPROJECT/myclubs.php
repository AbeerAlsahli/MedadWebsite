...
function leaveClub($userID, $clubID, $pdo) {
    $sql = "DELETE FROM club_memberships WHERE userID = ? AND clubID = ?";
    $stmt = $pdo->prepare($sql);
    
    if ($stmt->execute([$userID, $clubID])) {
        $_SESSION['success_message'] = 'تم مغادرة النادي بنجاح';
    } else {
        $_SESSION['error_message'] = 'حدث خطأ أثناء مغادرة النادي';
    }
}

// Get user's club memberships
$memberships_sql = "
    SELECT c.clubID, c.name, c.description, c.location, cm.userID 
    FROM club c 
    JOIN club_memberships cm ON c.clubID = cm.clubID 
    WHERE cm.userID = ?";
$memberships_stmt = $pdo->prepare($memberships_sql);
$memberships_stmt->execute([$userID]);
$memberships = $memberships_stmt->fetchAll();

// Count user's clubs
$club_count = count($memberships);
?>
<!DOCTYPE html>
    ...
    <style>
        ...
        * {
        ...
        @media (prefers-reduced-motion: reduce) {
            .club-card, .leave-club-btn, .hamburger, .close-btn, .menu-content a {
                transition: none;
            }
...
<body>
    ...
    <nav class="nav-menu" id="navMenu">
        ...
        <div class="menu-content">
            <a href="homep.html">الملف الشخصي</a>
    ...
    <!--  المحتوى الرئيسي  -->
    <div class="container">
        <h1 class="page-title">أنديتي</h1>
        <p class="page-subtitle">الأندية الأدبية التي انضممت إليها في منصة مِـداد</p>
        
        <div class="membership-stats-simple">
            <div class="stat-simple">
                <div class="stat-number-simple"><?php echo $club_count; ?></div>
                <div class="stat-text-simple">نادي منضم</div>
            </div>
        </div>

        <div class="clubs-container" id="my-clubs">
            <?php if (count($memberships) > 0): ?>
    <?php foreach($memberships as $club): ?>
                    <div class="club-card">
                        <div class="club-header">
                            <img src="image/event<?php echo $club['clubID']; ?>.jpg" alt="<?php echo $club['name']; ?>">
                            <div class="club-badge">نادي</div>
                        ...
                        <div class="club-content">
                            ...
                            <button class="club-details-btn">التفاصيل ▼</button>
                            <div class="club-details-content">
                                <p><strong>الموقع:</strong> <?php echo $club['location']; ?></p>
                                <p><strong>الاجتماعات:</strong> 
                                    <?php 
                                    // Generate meeting times based on club ID
                                    $meeting_times = [
                                        1 => "كل يوم ثلاثاء، 6:00 مساءً",
                                        2 => "كل يوم أحد، 5:00 مساءً", 
                                        3 => "كل يوم سبت، 4:00 مساءً"
                                    ];
                                    echo isset($meeting_times[$club['clubID']]) ? $meeting_times[$club['clubID']] : "سيتم الإعلان لاحقاً";
                                    ?>
                                </p>
                                <p><strong>تاريخ الانضمام:</strong> 
                                    <?php 
                                    // Generate join dates based on club ID
                                    $join_dates = [
                                        1 => "15 سبتمبر 2024",
                                        2 => "2 أكتوبر 2024",
                                        3 => "20 نوفمبر 2024"
                                    ];
                                    echo isset($join_dates[$club['clubID']]) ? $join_dates[$club['clubID']] : date("Y-m-d");
                                    ?>
                                </p>
                                <p><strong>النشاط:</strong> 
                                    <?php 
                                    // Generate activities based on club ID
                                    $activities = [
                                        1 => "جلسات حوارية، أمسيات شعرية، ورش عمل أدبية",
                                        2 => "ورش خط عربي، محاضرات عن تطور الخط، معارض",
                                        3 => "ندوات أدبية، مسابقات كتابة، نشر أعمال الأعضاء"
                                    ];
                                    echo isset($activities[$club['clubID']]) ? $activities[$club['clubID']] : "أنشطة أدبية متنوعة";
                                    ?>
                                </p>
                                <p><?php echo $club['description']; ?></p>
                            </div>
                            <div class="club-actions">
                                <form method="POST" style="width: 100%;">
                                    ...
                                    <button type="submit" class="leave-club-btn" onclick="return confirm('هل أنت متأكد من مغادرة نادي <?php echo $club['name']; ?>؟')">
                                        مغادرة
                                        <svg class="leave-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <?php else: ?>
                <div class="empty-state" id="empty-clubs">
                    <i>🏛️</i>
                    <h3>لم تنضم إلى أي أندية بعد</h3>
                    <p>انضم إلى الأندية الأدبية لاكتشاف مجتمع من القراء والكتاب المميزين</p>
                    <a href="events.php" class="explore-clubs-btn">استكشاف الأندية</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('club-details-btn')) {
